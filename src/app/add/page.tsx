...
// ── NY: farger for barn-kategori ─────────────────────────────────────────────
const COLOR_OPTIONS = [
  { label: 'Hvit',      hex: '#F5F5F0' },
  { label: 'Grå',       hex: '#9E9E9E' },
  { label: 'Svart',     hex: '#2C2C2C' },
  { label: 'Blå',       hex: '#3B82F6' },
  { label: 'Grønn',     hex: '#4A7C59' },
  { label: 'Rød',       hex: '#C4322A' },
  { label: 'Rosa',      hex: '#F4A0B0' },
  { label: 'Gul',       hex: '#F5C842' },
  { label: 'Beige',     hex: '#D4B896' },
  { label: 'Flerfarge', hex: 'conic-gradient(red, yellow, green, blue, red)' },
]
...
function AddPageContent() {
  ...
  // ── NY: produktsøk-state (kun for barn) ──────────────────────────────────
  const [selectedProduct, setSelectedProduct] = useState<ProductResult | null>(null)
  const [barnColor, setBarnColor] = useState<string | null>(null)
  ...
  // ── NY: håndter produktvalg fra ProductSearch ─────────────────────────────
  const handleProductSelect = (product: ProductResult) => {
    setSelectedProduct(product)
    setName(product.name)
    setDescription([
      product.brand ? `Merke: ${product.brand}` : '',
      product.description,
    ].filter(Boolean).join('\n'))
    if (product.imageUrl) {
      setSuggestedImageUrl(product.imageUrl)
      setSelectedImage('suggested')
...
Returner KUN JSON, ingen annen tekst.` }
  ...
  const saveItem = async () => {
    ...
    // ── NY: legg farge inn i beskrivelsen for barn ────────────────────────
    const finalDescription = category === 'barn' && barnColor
      ? [description, `Farge: ${barnColor}`].filter(Boolean).join('\n')
      : description
  ...
  return (
    <div className="max-w-lg mx-auto pb-24">
      ...
      <div className="px-4 pt-5 flex flex-col gap-4">
        ...
        {/* ── NY: Produktsøk-blokk — vises kun når kategori = barn ── */}
        {category === 'barn' && (
          <div className="flex flex-col gap-3">
            ...
            {!selectedProduct ? (
              ...
              <div className="flex items-center gap-3 rounded-2xl px-4 py-3"
                style={{ background: 'rgba(196,103,58,0.06)', border: '1px solid rgba(196,103,58,0.2)' }}>
                {suggestedImageUrl
                  ? <img src={suggestedImageUrl} alt={name}
                      style={{ width: 44, height: 44, borderRadius: 8, objectFit: 'cover', flexShrink: 0 }} />
                  : <div style={{ width: 44, height: 44, borderRadius: 8, background: 'rgba(196,103,58,0.1)', flexShrink: 0 }} />
                }
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--terra-dark)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</p>
                  <p style={{ fontSize: 11, color: 'var(--terra)' }}>✓ Hentet automatisk</p>
                </div>
                <button onClick={resetProductSearch}
                  style={{ fontSize: 12, color: 'var(--terra-mid)', background: 'transparent', border: 'none', cursor: 'pointer', padding: '4px 8px' }}>
                  Bytt
                </button>
              </div>
            )}
          ...
          <>
            <div className="flex flex-col gap-1">
              ...
              <div className="flex flex-wrap gap-2">
                {CATEGORIES.map(cat => (
                  <button key={cat.id} onClick={() => { setCategory(cat.id); setSubcategory(''); if (cat.id !== 'barn') { resetProductSearch() } }}
            ...
            {/* ── NY: Fargevalg — vises kun for barn ── */}
            {category === 'barn' && (
              <div className="flex flex-col gap-2">
                <label className="text-xs font-medium uppercase tracking-wide" style={{ color: 'var(--terra-mid)' }}>Farge (valgfritt)</label>
                <div className="flex flex-wrap gap-2">
                  {COLOR_OPTIONS.map(c => (
                    <button
                      key={c.label}
                      onClick={() => setBarnColor(barnColor === c.label ? null : c.label)}
                      style={{
                        display: 'flex', alignItems: 'center', gap: 6,
                        padding: '5px 10px', borderRadius: 20, fontSize: 12,
                        border: barnColor === c.label
                          ? '1.5px solid rgba(196,103,58,0.5)'
                          : '1px solid rgba(196,103,58,0.18)',
                        background: barnColor === c.label
                          ? 'rgba(196,103,58,0.1)'
                    ...
                    >
                      <span style={{
                        ...
                        background: c.hex,
                        border: c.label === 'Hvit' ? '1px solid rgba(0,0,0,0.12)' : 'none',
                        flexShrink: 0,
                      }} />
                      {c.label}
                    </button>
