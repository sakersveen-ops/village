// Path of this file: src/app/friends/page.tsx
'use client'
    ...
    <div className="max-w-lg mx-auto pb-24">
      <div className="sticky top-0 bg-[var(--glass-bg-heavy)] border-b border-[var(--glass-border)] px-4 pt-10 pb-4 z-10">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-[var(--terra-dark)]">Venner</h1>
          {pendingRequests.length > 0 && (
            <span className="bg-[var(--terra)] text-white text-xs font-bold px-2 py-0.5 rounded-full">
        ...
        <div className="relative mt-3">
          ...
          <input
            ...
            className="w-full bg-white border border-[var(--glass-border)] rounded-xl pl-10 pr-4 py-2.5 text-[var(--terra-dark)] outline-none focus:border-[var(--terra)] text-sm"
          />
        </div>
      </div>
      ...
      <div className="px-4 pt-4 flex flex-col gap-5">
              ...
              <div key={result.id} className="bg-white rounded-2xl px-4 py-3 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-[var(--terra-dark)] text-sm">{displayName(result)}</p>
                    {result.username && <p className="text-xs text-[var(--terra-mid)]">@{result.username}</p>}
                    {mutualMap[result.id]?.length > 0 && (
                      <button onClick={() => setExpandedMutual(expandedMutual === result.id ? null : result.id)} className="text-xs text-[var(--terra)] mt-0.5">
                  ...
                  {result.isFriend ? (
                    ...
                    <span className="text-xs text-[var(--terra-mid)] flex-shrink-0">Sendt</span>
                  ) : (
                    <button onClick={() => sendFriendRequest(result.id)} className="text-xs bg-[var(--terra)] text-white rounded-full px-3 py-1.5 font-medium flex-shrink-0">
                      + Legg til
                    </button>
                  )}
                </div>
                {expandedMutual === result.id && mutualMap[result.id]?.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-[var(--glass-border)] flex flex-wrap gap-2">
                    {mutualMap[result.id].map((m: any) => (
                      <div key={m.id} className="flex items-center gap-1.5 bg-[var(--glass-bg-heavy)] rounded-full px-2 py-1">
                        <div className="w-5 h-5 rounded-full bg-[var(--glass-border)] flex items-center justify-center text-xs font-bold overflow-hidden">
                          {m.avatar_url ? <img src={m.avatar_url} className="w-full h-full object-cover" /> : m.name?.[0]?.toUpperCase()}
                        </div>
                        <span className="text-xs text-[#1A3542]">{m.name || m.email?.split('@')[0]}</span>
                      </div>
                    ))}
                  </div>
                )}
        ...
        {pendingRequests.length > 0 && (
          <div>
            <h2 className="text-sm font-bold text-[var(--terra-dark)] mb-2">Venneforespørsler</h2>
            <div className="flex flex-col gap-2">
              {pendingRequests.map(req => (
                <div key={req.id} className="bg-white rounded-2xl px-4 py-3 flex items-center gap-3 shadow-sm">
                  ...
                  <p className="flex-1 font-medium text-[var(--terra-dark)] text-sm">{displayName(req.profiles)}</p>
                  ...
                  <button onClick={() => respondToFriendRequest(req.id, req.from_id, false)} className="text-xs border border-[var(--glass-border)] text-[var(--terra-mid)] rounded-full px-3 py-1.5">Avslå</button>
                </div>
              ))}
            </div>
          </div>
        ...
        {sentRequests.length > 0 && (
          <div>
            <h2 className="text-sm font-bold text-[var(--terra-dark)] mb-2">Venter på svar</h2>
            <div className="flex flex-col gap-2">
              {sentRequests.map(req => (
                <div key={req.id} className="bg-white rounded-2xl px-4 py-3 flex items-center gap-3 shadow-sm">
                  <div className="w-10 h-10 rounded-full bg-[var(--glass-border)] flex items-center justify-center font-bold text-sm overflow-hidden flex-shrink-0">
                    {req.profiles?.avatar_url
                      ? <img src={req.profiles.avatar_url} className="w-full h-full object-cover" />
                      : displayName(req.profiles)?.[0]?.toUpperCase()}
                  </div>
                  <div className="flex-1">
                    <p className="font-medium text-[var(--terra-dark)] text-sm">{displayName(req.profiles)}</p>
                    {req.profiles?.username && <p className="text-xs text-[var(--terra-mid)]">@{req.profiles.username}</p>}
                  </div>
                  <button onClick={() => cancelFriendRequest(req.id)} className="text-xs border border-[var(--glass-border)] text-[var(--terra-mid)] rounded-full px-3 py-1.5">
                    Trekk tilbake
                  </button>
                </div>
              ))}
        ...
        {/* Venneliste */}
        <div>
          <h2 className="text-sm font-bold text-[var(--terra-dark)] mb-2">
            Mine venner {friends.length > 0 && <span className="text-[var(--terra-mid)] font-normal">({friends.length})</span>}
          </h2>
          {friends.length === 0 ? (
            <div className="bg-white rounded-2xl p-5 text-center text-[var(--terra-mid)] text-sm">
              Ingen venner ennå – <Link href="/invite" className="text-[var(--terra)]">inviter noen!</Link>
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              {sortedFriends.map((f: any) => (
                <Link key={f.user_b} href={`/profile/${f.user_b}`}>
                  <div className="bg-white rounded-2xl px-4 py-3 flex items-center gap-3 shadow-sm">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-[var(--terra-dark)] text-sm">{displayName(f.profiles)}</p>
                      {f.profiles?.username && <p className="text-xs text-[var(--terra-mid)]">@{f.profiles.username}</p>}
                    </div>
                    <button
        ...
        <Link href="/invite">
          <div className="bg-[var(--terra)] rounded-2xl p-4 flex items-center justify-between">
            <div>
