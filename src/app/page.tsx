...
export default function FeedPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Fetch all available items visible to the user (RLS controls access)
      const { data: items } = await supabase
        .from('items')
        .select('*, profiles(id, name, avatar_url)')
        .neq('owner_id', user.id)       // exclude own items from feed
        .order('created_at', { ascending: false })
        .limit(60)

      // Sort: friends' items first, then others
                ...
                {sortedCategories.map(cat => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        'relative rounded-[16px] p-3 flex flex-col items-center gap-1.5 border transition-all duration-200',
                        isActive ? 'bg-[var(--terra)] border-[var(--terra)] shadow-sm' : 'glass border-[rgba(196,103,58,0.18)]',
