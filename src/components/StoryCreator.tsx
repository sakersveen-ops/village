...
interface Item {
  ...
  category: string
  available: boolean
  next_available?: string | null  // earliest due_date from active loans
}
...
export default function StoryCreator({ onClose, onCreated }: StoryCreatorProps) {
  ...
  // Cover step
  const [coverUrl, setCoverUrl] = useState<string | null>(null)      // final url (uploaded or default)
  const [coverPreview, setCoverPreview] = useState<string | null>(null) // local blob preview
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Fetch next available date for unavailable items
      ...
      if (unavailableIds.length > 0) {
        ...
        // Keep earliest due_date per item
        for (const loan of loans || []) {
          if (!nextMap[loan.item_id]) nextMap[loan.item_id] = loan.due_date
        }
      }

      setMyItems(itemRows.map(i => ({
        ...i,
        next_available: nextMap[i.id] ?? null,
      })))
    }
    load()
  }, [])
  ...
  const visibleItems = categoryFilter
    ? myItems.filter(i => i.category === categoryFilter)
    : myItems
  ...
  // ── Save story (called when entering cover step) ──
  const saveStoryAndSlides = async (): Promise<boolean> => {
    ...
    const { data: story, error: storyErr } = await supabase
      ...
      .insert({
        ...
        type: 'custom',
        category: null,
        cover_url: null,   // updated after cover step
        cover_text: null,  // updated after cover step
      })
      .select('id')
      .single()
  ...
  const handleBack = () => {
    ...
    // cover step: story already saved, can't truly go back — just close
    if (step === 'cover') { onCreated(); return }
  }
  ...
  return (
    <div
      ...
      style={{ background: '#111', maxWidth: 480, margin: '0 auto' }}
    >
      {/* ── Header ── */}
      <div
      ...
      >
        <button
          ...
          aria-label={step === 'select' ? 'Lukk' : 'Tilbake'}
        >
          {step === 'select'
      ...
      {/* ════════════════════════════════════════
          STEP: select (name + category filter + items)
          ════════════════════════════════════════ */}
      {step === 'select' && (
        <div className="flex-1 flex flex-col min-h-0">

          {/* ── Sticky top: name + category pills ── */}
          <div
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3">
            {visibleItems.length === 0 ? (
              <p className="text-sm text-center py-8" style={{ color: 'rgba(255,255,255,0.4)' }}>
                Ingen gjenstander i denne kategorien
              </p>
            ) : (
                ...
                {visibleItems.map(item => {
                  ...
                  return (
                    <div key={item.id}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.4)' }}>
                            {item.available
                              ? 'Ledig'
                              : item.next_available
                                ? `Ledig fra ${formatDate(item.next_available)}`
                                : 'Utlånt'
                            }
                          </p>
                        </div>
                      ...
                      {/* Caption toggle — only when selected */}
                      {isSelected && (
                        <div
                          style={{
                            background: 'rgba(196,103,58,0.12)',
                            border: '1.5px solid rgba(196,103,58,0.5)',
                            borderTop: 'none',
          ...
          >
            ...
            <button
              ...
              style={{
                background: selected.length > 0 && title.trim() ? 'var(--terra)' : 'rgba(255,255,255,0.12)',
                color: '#fff',
              }}
            >
              {selected.length > 0
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="flex-1 overflow-y-auto px-4 py-3">
            <p className="text-xs mb-3" style={{ color: 'rgba(255,255,255,0.4)' }}>
              Dra for å sortere. Første gjenstand vises øverst i storyen.
            </p>
            <div className="flex flex-col gap-2 pb-2">
                ...
                >
                  ...
                  <div className="flex flex-col gap-1 flex-shrink-0">
                    <button onClick={() => moveUp(idx)} disabled={idx === 0}
                      style={{ opacity: idx === 0 ? 0.2 : 1, color: 'rgba(255,255,255,0.6)' }} aria-label="Opp">
                    ...
                    <button onClick={() => moveDown(idx)} disabled={idx === orderedItems.length - 1}
                      style={{ opacity: idx === orderedItems.length - 1 ? 0.2 : 1, color: 'rgba(255,255,255,0.6)' }} aria-label="Ned">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="flex-1 overflow-y-auto px-4 py-4">
            <p className="text-xs mb-4" style={{ color: 'rgba(255,255,255,0.4)' }}>
              Velg forsidebilde og legg til tekst. Standard er det første bildet i storyen.
            </p>

            {/* Cover preview */}
            <div
              className="relative w-full rounded-2xl overflow-hidden mb-4 flex items-center justify-center"
              style={{ aspectRatio: '1/1', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}
            >
              {(coverPreview ?? defaultCoverImg) ? (
                <img
                  src={coverPreview ?? defaultCoverImg!}
              ...
              {coverText.trim() && (
                ...
                >
                  <p className="font-display text-lg font-bold" style={{ color: '#fff', letterSpacing: '-0.02em' }}>
                    {coverText}
                  </p>
                </div>
              )}
            ...
            <div className="mb-2">
              ...
              <input
                ...
                placeholder={`F.eks. «${title || 'Min story'}»`}
                maxLength={60}
